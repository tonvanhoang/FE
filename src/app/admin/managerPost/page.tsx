...
export default function ManagerPost() {
  ...
  useEffect(() => {
    fetchReport();
    socket.on("newReport", async (reportData: any) => {
      const resPostDetail = await fetch(`http://localhost:4000/post/postByID/${reportData.idPost}`);
      const dataPost = await resPostDetail.json();
      reportData.postDetails = dataPost;
      setReports((prevReports: any) => [reportData, ...prevReports]);
    });
  ...
  const btnDeletePost = async (id: string, idre: string) => {
    ...
    if (res.ok) {
      await fetch(`http://localhost:4000/report/delete/${idre}`, {
        headers: {
          'content-type': 'application/json'
        },
        method: 'DELETE'
              ...
              {reports.map((report: any) => (
                <tr key={report._id} className="trAccount">
                  ...
                  <td>
                    ...
                    {
                      report.statusReport === 'Chưa xác nhận' && (
